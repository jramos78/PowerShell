...
function R53Record {  
  #Get the names of every Route 53 hosted zone 
	...
	for ($i = 1;$i -le $zones.Length;$i++){  
		switch ($selection){$i {$r53Zone = $zones[$i-1];Break}}  
	}
  ...
  if ($selection -eq "q"){  
    ...
    if (($selection -lt 0) -Or ($selection -gt $zones.Length)){Write-Warning "The script has been terminated because an invalid value was entered!";Break}  
  }
  ...
  $selection = Read-Host "`nPlease make a selection"
	...
	if ($selection -eq 1){
	  $recordType = $types[0]
	} elseif ($selection -eq 2){$recordType = $types[1]}
	#Exit the function if the user enters "q", "Q" or an invalid number.   
  if ($selection -eq "q"){  
    Write-Warning "The script was terminated by the user!"  
    Break 
